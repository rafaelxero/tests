...
void CFSImpl::extractConstraintPoints(const CollisionPair& collisionPair)
{
    ...
    if(p != geometryPairToLinkPairMap.end()){
        ...
        cout << "muStatic: " << linkPair.attr.muStatic << std::endl;
        cout << "muDynamic: " << linkPair.attr.muDynamic << std::endl;
        cout << "epsilon: " << linkPair.attr.epsilon << std::endl;
        pLinkPair = &linkPair;
    }
        ...
        for(int j=0; j < numConstraintsInPair; ++j){
            ...
            if(linkPair.isNonContactConstraint){
                ...
                if(ENABLE_CONTACT_DEPTH_CORRECTION){
                    ...
                    b(globalIndex) = an0(globalIndex) + ((1 + linkPair.attr.epsilon)*constraint.normalProjectionOfRelVelocityOn0 - velOffset) * dtinv;
                } else {
                    b(globalIndex) = an0(globalIndex) + (1 + linkPair.attr.epsilon)*constraint.normalProjectionOfRelVelocityOn0 * dtinv;
                }
